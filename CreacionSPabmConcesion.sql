...
begin
	...
	BEGIN
		...
		SET @errorLine = @errorLine + CHAR(13) + 'ERROR: Nombre invalido.'
	END

	if (@descripcion IS NULL)
	BEGIN
		SET @errorCount = @errorCount + 1
		SET @errorLine = @errorLine + CHAR(13) + 'ERROR: Descripcion invalida.'
	END

	IF( (@errorCount = 0) AND EXISTS(SELECT 1 FROM PnTablas.Empresa WHERE NombreEmpresa LIKE @nombre) )
	BEGIN
		SET @errorCount = @errorCount + 1
		SET @errorLine = @errorLine + CHAR(13) + 'ERROR: Empresa ya presente.'
	END
...
begin
	...
	BEGIN
		SET @errorCount = @errorCount + 1
		SET @errorLine = @errorLine + CHAR(13) + 'ERROR: Nuevo nombre invalido.'
	END
	...
	IF( (@errorCount = 0) AND EXISTS(SELECT 1 FROM PnTablas.Empresa WHERE IDEmpresa != @idEmpresa AND NombreEmpresa LIKE @nombre) )
	BEGIN
		SET @errorCount = @errorCount + 1
		SET @errorLine = @errorLine + CHAR(13) + 'ERROR: Ya hay otra empresa con ese nombre.'
	END
...
begin
	...
	BEGIN
		SET @errorCount = @errorCount + 1
		SET @errorLine = @errorLine + CHAR(13) + 'ERROR: Nueva descripcion es invalida.'
	END
